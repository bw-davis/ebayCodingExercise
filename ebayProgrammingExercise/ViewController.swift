...
class CountriesTableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.getAndParseData()                      //call function to get and parse the data from API
        self.theTableView.reloadData()              //reload the table view once data is obtained
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "CountryDetailSegue" {       //check for proper segue
            guard let indexPath = theTableView.indexPathForSelectedRow else {       //ensure we have index path
                return          //if no index path return out of function
            }
            /*create create instance of destination view controller and sets the selected country variable to the country that was pressed*/
